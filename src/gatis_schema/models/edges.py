...
class RoadEdge(EdgeBase):
    ...
    surface_issue: Annotated[Omitable[str], Tier("optional")] = Field(description="yes, no, cracking, scaling, spalling, uneven, frequent water pooling, heaving, missing bricks/stones, potholes/holes, slickness, detectable warning surface damage, longitudinal cracks and seams, other Recommended values: yes; no; cracking; scaling; spalling; uneven; frequent water pooling; heaving; missing bricks/stones; potholes/holes; slickness; detectable warning surface damage; longitudinal cracks and seams; metal plates; other.")
...
class SidewalkEdge(EdgeBase):
    ...
    separation_elements: Annotated[Omitable[list[str]], Tier("optional", {3: "recommended"})] = Field(description="The materials used to separate the cycleway or footpath from motor vehicle traffic -- for example, as part of a buffer. Recommended values: bollards; concrete barrier; parking; median; trees.")
    ...
    surface_issue: Annotated[Omitable[str], Tier("optional", {3: "recommended", 4: "required"})] = Field(description="yes, no, cracking, scaling, spalling, uneven, frequent water pooling, heaving, missing bricks/stones, potholes/holes, slickness, detectable warning surface damage, longitudinal cracks and seams, other Recommended values: yes; no; cracking; scaling; spalling; uneven; frequent water pooling; heaving; missing bricks/stones; potholes/holes; slickness; detectable warning surface damage; longitudinal cracks and seams; metal plates; other.")
...
class CrossingEdge(EdgeBase):
    ...
    separation_elements: Annotated[Omitable[list[str]], Tier("optional")] = Field(description="The materials used to separate the cycleway or footpath from motor vehicle traffic -- for example, as part of a buffer. Recommended values: bollards; concrete barrier; parking; median; trees.")
...
class BikewayEdge(EdgeBase):
    ...
    bikeway_type: Annotated[str, Tier("required")] = Field(description="Common name used for the bicycle facility type. Should align with the National Bikeway Network, NACTO, or AASHTO facility types. Recommended values: Bike Lane; Buffered Bike Lane; Separated Bike Lane; Counter-Flow Bike Lane; Paved Shoulder; Shared Lane.")
    ...
    separation_elements: Annotated[Omitable[list[str]], Tier("optional", {1: "recommended", 2: "required"})] = Field(description="The materials used to separate the cycleway or footpath from motor vehicle traffic -- for example, as part of a buffer. Recommended values: bollards; concrete barrier; parking; median; trees.")
