...
class VirtualNode(NodeBase):
    """Used to create nodes for routing in places where specific physical infrastructure
    doesn’t exist, where edges are split because attributes are different between them,
    or where other nodes don’t suit the purpose.
    """

    node_type: Annotated[Literal["virtual"], Tier("required")] = Field(
    ...
    rail_crossing: Annotated[
        Omitable[list[RailCrossing]], Tier("optional", {3: "recommended"})
    ] = Field(
...
class CurbRampNode(NodeBase):
    """Indicates a location where a pedestrian must make a decision about which
    direction to travel, most commonly in locations where a curb ramp does or should
    exist.
    """
    ...
    date_built: Annotated[Omitable[GatisDate], Tier("optional", {2: "recommended"})] = (
        Field(
            description="When the facility was officially opened for use. If the "
            "facility has had a major remodeling where the structure, shape or another "
            "fundamental aspect was changed, the date of remodeling can be placed "
            "here. Report in RFC 3339 format containing day, month and year, or just "
            "month and year or year if day or month is not available."
        )
    )

    check_date: Annotated[Omitable[GatisDate], Tier("optional", {2: "recommended"})] = (
        Field(
    ...
    incline: Annotated[Omitable[Percent], Tier("optional", {3: "required"})] = Field(
    ...
    width: Annotated[Omitable[InchesFloat], Tier("optional", {3: "required"})] = Field(
    ...
    ramp_type: Annotated[Omitable[str], Tier("optional", {3: "recommended"})] = Field(
        ...
        "diagonal; parallel; perpendicular; unknown."
    )

    detectable_warning: Annotated[
        Omitable[DetectableWarning], Tier("optional", {3: "required"})
    ] = Field(
    ...
    impediment: Annotated[
        Omitable[list[Impediment]], Tier("optional", {3: "recommended", 4: "required"})
    ] = Field(
...
class ElevatorNode(NodeBase):
    """Elevators, funiculars, or other car- or enclosure-based constructions meant to
    vertically carry a pedestrian from one level of physical infrastructure to another.
    """

    node_type: Annotated[Literal["elevator"], Tier("required")] = Field(
        description="Indicates the type of node."
    )
    ...
    status: Annotated[
        Omitable[Status], Tier("optional", {3: "recommended", 4: "required"})
    ] = Field(
        description="Most recent operating status of the node. Whether the "
        "infrastructure is open and available for use. Default is 'open'"
    )
...
class TransitStopNode(NodeBase):
    ...
    node_type: Annotated[Literal["transit_stop"], Tier("required")] = Field(
        description="Indicates the type of node."
    )
    ...
    gtfs_id: Annotated[
        Omitable[list[GtfsReference]], Tier("optional", {3: "recommended"})
    ] = Field(
        description="An array of objects containing keys for GTFS agency_id and "
        "stop_id from an existing GTFS dataset for this transit stop. Represent as "
        "string with the format '{agency_id},{stop_id}'. These IDs can be "
        "crosswalked with GTFS and TIDES data to obtain additional stop attributes "
        "and connect to the broader transit network. See GTFS documentation for "
        "more information."
    )


class IssueNode(NodeBase):
    """A node indicating a particular point that has an issue relevant to routing for
    some travelers.
    """

    node_type: Annotated[Literal["issue"], Tier("required")] = Field(
        description="Indicates the type of node."
    )
...
class TrafficCalmingNode(NodeBase):
    ...
    node_type: Annotated[Literal["traffic_calming"], Tier("required")] = Field(
        description="Indicates the type of node."
    )
    ...
    traffic_calming_type: Annotated[
        Omitable[str], Tier("optional", {3: "recommended", 4: "required"})
    ] = Field(
        description="Used to identify traffic calming features on a road or "
        "crossing. Used when it makes more sense to model the traffic calming "
        "feature as a node instead of an edge. Recommended values are from "
